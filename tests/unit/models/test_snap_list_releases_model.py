        ...
        "revisions": [
            {
                ...
                "build-url": "https://github.com/actions/build/1",
                "revision": 5,
...
def test_get_list_releases(payload):
    model = snap_list_releases_model.ListReleasesModel(**payload)

    assert len(model.channel_map) == 1
    assert model.channel_map[0].architecture == "amd64"
    ...
    assert model.revisions[0].architectures == ["amd64"]
    assert model.revisions[0].base == "core22"
    assert model.revisions[0].created_at == datetime.datetime(
        2021, 11, 19, 11, 55, 0, 84876
    )
    assert model.revisions[0].created_by == "developer"
    assert model.revisions[0].build_url == "https://github.com/actions/build/1"
    ...
    assert model.revisions[0].confinement == "strict"
    assert model.revisions[0].grade == "stable"
    assert model.revisions[0].type == "app"
