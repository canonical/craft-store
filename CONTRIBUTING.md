...
professional, and get involved in the growing open source community. Craft Store
...
GitHub issue. The Craft Store maintainers review all proposals for technical soundness.
...
```bash
git clone git@github.com:<username>/craft-store --recurse-submodules
cd craft-store
git remote add upstream git@github.com:canonical/craft-store
git fetch upstream
```
...
```bash
git clone https://github.com/<username>/craft-store --recurse-submodules
cd craft-store
git remote add upstream https://github.com/canonical/craft-store
git fetch upstream
```
...
Craft Store maintainer can assign the ticket to you before starting. This process
...
Craft Store maintainer will assign it to you.

If you don't find a related issue, [open
one](https://github.com/canonical/craft-store/issues/new/choose) and indicate that
you're interested in taking it on. When creating the issue, be sure to add any relevant
labels. A Craft Store maintainer will then review the issue and assign it to you.
...
[Diátaxis](https://diataxis.fr) category for the page. A Craft Store maintainer will
...
Craft Store's test suite includes both unit and integration tests. If you're not sure
...
Store's documentation is built around.
...
Finally, [open a PR](https://github.com/canonical/craft-store/compare) for it on
...
The Craft Store maintainers try to review every PR in a timely manner, typically
